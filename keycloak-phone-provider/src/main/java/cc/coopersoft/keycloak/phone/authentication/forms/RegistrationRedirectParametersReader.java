...
public class RegistrationRedirectParametersReader implements FormActionFactory, FormAction {
  ...
  public void success(FormContext context) {
    ...
    URI url = URI.create(redirectUri);
      ...
      Map<String, List<String>> queryParams =
              Arrays.stream(query.split("&"))
                      ...
                      .collect(Collectors.groupingBy(
                              ...
                              Collectors.mapping(pair -> URLDecoder.decode(pair[1], StandardCharsets.UTF_8), Collectors.toList())
                      ));

      for (String paramName : queryParams.keySet()) {
        if (finalParamNames.contains(paramName)) {
          List<String> paramValues = queryParams.get(paramName);
          user.setAttribute(paramName, paramValues);
        }
      }
    }
